...
import the_fireplace.grandeconomy.utils.TextPaginationUtil;
    ...
    public static void register(CommandDispatcher<ServerCommandSource> commandDispatcher) {
        ...
        commandDispatcher.register(CommandManager.literal("pay").requires((iCommandSender) -> iCommandSender.getEntity() instanceof PlayerEntity)
                        ...
                        .executes((command) -> {
                            ...
                            targetPlayer.sendMessage(TranslationUtil.getTranslation(targetPlayer.getUuid(), "commands.grandeconomy.pay.recieved", GrandEconomyApi.formatCurrency(amount), command.getSource().getName()), false);
                            return Command.SINGLE_SUCCESS;
                        }))));
        ...
        commandDispatcher.register(CommandManager.literal("gehelp").requires((iCommandSender) -> iCommandSender.hasPermissionLevel(0))
                .executes((command) -> {
                    TextPaginationUtil.sendPaginatedChat(command.getSource(), "/gehelp %s", getHelpsList(command), 1);
                    return Command.SINGLE_SUCCESS;
                })
                .then(CommandManager.argument("page", IntegerArgumentType.integer(1))
                .executes((command) -> {
                    TextPaginationUtil.sendPaginatedChat(command.getSource(), "/gehelp %s", getHelpsList(command), command.getArgument("page", Integer.class));
                    return Command.SINGLE_SUCCESS;
                })));
    }
