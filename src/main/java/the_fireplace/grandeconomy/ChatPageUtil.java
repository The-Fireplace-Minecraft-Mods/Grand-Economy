...
public class ChatPageUtil {
    ...
    public static void showPaginatedChat(ICommandSender sender, String command, List<ITextComponent> items, int page) {
        ...
        ITextComponent nextButton = current < totalPageCount ? TranslationUtil.getTranslation(sender, "grandeconomy.chat.page.next").setStyle(new Style().setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, String.format(command, current+1)))) : new TextComponentString("-----");
        ITextComponent prevButton = current > 1 ? TranslationUtil.getTranslation(sender, "grandeconomy.chat.page.prev").setStyle(new Style().setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, String.format(command, current-1)))) : new TextComponentString("------");
