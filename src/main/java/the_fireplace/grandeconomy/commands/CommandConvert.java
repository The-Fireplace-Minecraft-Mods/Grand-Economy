    ...
    public void execute(@Nonnull MinecraftServer server, @Nonnull ICommandSender sender, @Nonnull String[] args) throws CommandException {
        if (sender instanceof EntityPlayer) {
            ...
            if(ConversionItems.hasValue(heldResource, heldMeta)) {
                double value = ConversionItems.getValue(heldResource, heldMeta);
                ...
                assert heldResource != null;
                sender.sendMessage(TranslationUtil.getTranslation(((EntityPlayer) sender).getUniqueID(), "commands.grandeconomy.convert.success", count, heldResource.toString(), value, value * count, GrandEconomyApi.getFormattedBalance(((EntityPlayer)sender).getUniqueID(), true)));
            } else
