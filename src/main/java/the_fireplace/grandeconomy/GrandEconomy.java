...
import the_fireplace.grandeconomy.ge.GrandEconomyEconHandler;
import the_fireplace.grandeconomy.requesthandler.GrandEconomyApiFabric;
...
public class GrandEconomy implements ModInitializer {
    ...
    private static final IEconHandler economyWrapper = new IEconHandler() {
        ...
        public boolean addToBalance(UUID uuid, double amount, Boolean isPlayer) {
            if(ModConfig.enforceNonNegativeBalance && amount < 0) {
    ...
    public void onInitialize() {
        ...
        ServerLifecycleEvents.SERVER_STARTED.register(s -> {
            minecraftServer = s;
            ModConfig.load();
            economy = GrandEconomyApi.getEconHandlers().getOrDefault(ModConfig.economyBridge, new GrandEconomyEconHandler());
